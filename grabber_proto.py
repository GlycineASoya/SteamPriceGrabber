...
def getPrice(page: requests.models.Response) -> str:
    ...
    if match is not None:
        ...
        if match is not None:
            ...
            print("The page " + page.url + " either agechecked or does not have price")
...
def getCcPage(url: str) -> requests.models.Response:
    for cc in cc_list:
        if isPageExist(url + "/?cc=" + cc):
            return getPage(url + "/?cc=" + cc)
        else:
            return None
...
game_list = list()  # list of dicts# [{"id": "<number>", "name": "<get_name()>", "price": "list {<get_price()>}",
# "datetime": "<timestamp>", "isExist": "<bool>"}]
cc_price = list()  # list of the prices for different countries

for page_id in existed_pages:
    if isPageExist(url + str(page_id)):
        page = getPage(url + str(page_id))
        for cc in cc_list:
            if isPageExist(url + str(page_id) + "?cc=" + cc):
                cc_price.append(getPrice(getPage(url + str(page_id) + "?cc=" + cc)))
        game_list.append(
            dict(datetime=str(date.today()), id=page_id, name=getName(getPage(url + str(page_id))),
                 price=cc_list, isExist="True")
        )
        #existed_pages.add(set(getIdsOnPage(page)))  # include this later
    else:
